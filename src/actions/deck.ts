...
export const createDeck = safeAction(
  "createDeck",
...
export const updateDeck = safeAction(
  "updateDeck",
  async (input: unknown): Promise<Result<{ id: string }>> => {
    const session = await requireSession();
    ...
    if (updates.viewPolicy !== undefined) {
      ...
      if (deck.linkedDeckDefinitionId) {
        return err(
          "BAD_REQUEST",
          "Linked decks cannot change visibility — only the source deck owner can do that",
        );
      }

      const perm = await requireFolderRole(deck.folderId, session.user.id, "admin");
      if (!perm.allowed)
        return err("PERMISSION_DENIED", "Only admins and owners can change deck visibility");
    }
...
export const getDeck = safeAction(
  "getDeck",
  async (deckId: string): Promise<Result<typeof deckDefinitions.$inferSelect>> => {
    if (!isValidUuid(deckId)) return err("VALIDATION_FAILED", "Invalid deck ID");
    const session = await requireSession();
    ...
    if (deck.archivedAt) {
      ...
      if (!canView) return err("NOT_FOUND", "The author has archived this deck");
    } else {
      const canView = await canViewDeck(deckId, session.user.id);
...
export const listDecks = safeAction(
  "listDecks",
  async (
    folderId: string,
...
export const archiveDeck = safeAction(
  "archiveDeck",
  async (deckId: string): Promise<Result<{ id: string }>> => {
    if (!isValidUuid(deckId)) return err("VALIDATION_FAILED", "Invalid deck ID");
    const session = await requireSession();

    if (session.user.defaultDeckId === deckId) {
      return err("BAD_REQUEST", "Your default deck cannot be archived.");
    }
    ...
    const perm = await requireFolderRole(deck.folderId, session.user.id, "admin");
    if (!perm.allowed)
      return err("PERMISSION_DENIED", "Only folder owners and admins can archive decks");
...
export const listEditableDecks = safeAction(
  "listEditableDecks",
  async (): Promise<
    Result<
      Array<{
...
export const searchLibraryDecks = safeAction(
  "searchLibraryDecks",
  async (query: string): Promise<Result<{ id: string; title: string }[]>> => {
    const session = await requireSession();
    ...
    const conditions = [
      inArray(deckDefinitions.folderId, fIds),
...
export const moveDeck = safeAction(
  "moveDeck",
  async (deckDefinitionId: string, targetFolderId: string): Promise<Result<{ id: string }>> => {
    if (!isValidUuid(deckDefinitionId)) return err("VALIDATION_FAILED", "Invalid deck ID");
    if (!isValidUuid(targetFolderId)) return err("VALIDATION_FAILED", "Invalid folder ID");
    ...
    if (session.user.defaultDeckId === deckDefinitionId)
      return err("BAD_REQUEST", "Your default deck cannot be moved.");
    if (deck.folderId === targetFolderId) return err("CONFLICT", "Deck is already in this folder");
    ...
    if (deck.linkedDeckDefinitionId) {
      ...
      if (existing)
        return err("CONFLICT", "The target folder already has a linked copy of this deck.");
    }

    const sourcePerm = await requireFolderRole(deck.folderId, session.user.id, "owner");
    if (!sourcePerm.allowed)
      return err("PERMISSION_DENIED", "You must be the owner of the source folder");

    const targetPerm = await requireFolderRole(targetFolderId, session.user.id, "owner");
    if (!targetPerm.allowed)
      return err("PERMISSION_DENIED", "You must be the owner of the target folder");
...
export const getDeckSummary = safeAction(
  "getDeckSummary",
  async (deckId: string): Promise<Result<DeckSummary>> => {
    if (!isValidUuid(deckId)) return err("VALIDATION_FAILED", "Invalid deck ID");
    const session = await requireSession();
