...
export const addDeckToLibrary = safeAction(
  "addDeckToLibrary",
  async (deckDefinitionId: string): Promise<Result<{ id: string }>> => {
...
export const getStudySession = safeAction(
  "getStudySession",
  async (
    userDeckId: string,
    opts?: { limit?: number },
  ...
  > => {
    if (!isValidUuid(userDeckId)) return err("VALIDATION_FAILED", "Invalid user deck ID");
    const session = await requireSession();
    ...
    if (!userDeck) return err("NOT_FOUND", "User deck not found");
    ...
    if (!deck) return err("NOT_FOUND", "Deck definition not found");
...
export const submitReview = safeAction(
  "submitReview",
  async (input: unknown): Promise<Result<{ userCardStateId: string; nextDueAt: string }>> => {
    const session = await requireSession();
    ...
    if (!cardState) return err("NOT_FOUND", "Card state not found");
    ...
    if (!userDeck) return err("PERMISSION_DENIED", "Permission denied");
...
export const listUserDecks = safeAction(
  "listUserDecks",
  async (): Promise<
    Result<
      Array<{
...
export const listLibraryDecks = safeAction(
  "listLibraryDecks",
  async (): Promise<Result<LibraryDeck[]>> => {
    const session = await requireSession();
    const nowIso = new Date().toISOString();
...
export const getDeckStudyStats = safeAction(
  "getDeckStudyStats",
  async (
    deckDefinitionId: string,
  ): Promise<
...
export const getCardStudyStates = safeAction(
  "getCardStudyStates",
  async (deckDefinitionId: string): Promise<Result<CardStudyState[]>> => {
    if (!isValidUuid(deckDefinitionId)) return err("VALIDATION_FAILED", "Invalid deck ID");
    const session = await requireSession();
...
export const getReviewHeatmapData = safeAction(
  "getReviewHeatmapData",
  async (): Promise<Result<{ date: string; count: number }[]>> => {
    const session = await requireSession();
...
export const getCustomStudySession = safeAction(
  "getCustomStudySession",
  async (
    input: unknown,
  ): Promise<
    Result<{
...
export const countCustomStudyCards = safeAction(
  "countCustomStudyCards",
  async (input: unknown): Promise<Result<{ cardCount: number; deckCount: number }>> => {
    const session = await requireSession();
    const parsed = CustomStudySchema.safeParse(input);
    if (!parsed.success) return err("VALIDATION_FAILED", "Validation failed");
...
export const advanceTime = safeAction(
  "advanceTime",
  async (userDeckId: string, minutes: number): Promise<Result<{ updated: number }>> => {
    if (process.env.NODE_ENV === "production")
      return err("BAD_REQUEST", "Not available in production");
    if (!isValidUuid(userDeckId)) return err("VALIDATION_FAILED", "Invalid user deck ID");
    if (minutes <= 0 || minutes > 525600)
      return err("VALIDATION_FAILED", "Minutes must be between 1 and 525600");
    const session = await requireSession();
...
export const getNewCardsPerDay = safeAction(
  "getNewCardsPerDay",
  async (deckDefinitionId: string): Promise<Result<number>> => {
    if (!isValidUuid(deckDefinitionId)) return err("VALIDATION_FAILED", "Invalid deck ID");
    const session = await requireSession();
...
export const updateNewCardsPerDay = safeAction(
  "updateNewCardsPerDay",
  async (deckDefinitionId: string, value: number): Promise<Result<{ updated: boolean }>> => {
    if (!isValidUuid(deckDefinitionId)) return err("VALIDATION_FAILED", "Invalid deck ID");
    if (!Number.isInteger(value) || value < 0 || value > 9999) {
      return err("VALIDATION_FAILED", "New cards per day must be an integer between 0 and 9999");
    }
    const session = await requireSession();
    ...
    if (!userDeck) return err("NOT_FOUND", "Deck not in your library");
