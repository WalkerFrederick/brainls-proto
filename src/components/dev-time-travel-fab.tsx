...
import { Clock, ChevronUp, ChevronDown, FastForward, X } from "lucide-react";
...
export function DevTimeTravelFab() {
  ...
  if (!open) {
    return (
      <button
        ...
        className="fixed bottom-4 left-4 z-50 flex h-12 w-12 items-center justify-center rounded-full bg-amber-500 text-white shadow-lg transition-transform hover:scale-105 active:scale-95"
  ...
  return (
    <div className="fixed bottom-4 left-4 z-50 w-72 rounded-xl border bg-popover p-4 shadow-2xl">
      ...
      {message && (
        <div className="mt-3 rounded-md bg-muted px-3 py-1.5 text-xs font-medium">{message}</div>
      )}
    </div>
  );
}
